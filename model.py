...
def generate_model(opt):
    ...
    if opt.model == 'c3d':
        ...
        if opt.model_depth == 50:
            ...
            model = resnext.resnext101(
                ...
                sample_size=opt.sample_size,
                sample_duration=opt.sample_duration,
                with_egogesture=opt.with_egogesture)
    ...
    if not opt.no_cuda:
        ...
        if opt.is_pretrained_model_has_dropout:            #####
            model.module.fc=nn.Sequential(
                              nn.Dropout(opt.resnext_dropout_prob),
                              nn.Linear(model.module.fc.in_features, model.module.fc.out_features))
            ...
            if isinstance(pretrain,dict) :                         ####
                  ...
                  if not opt.no_train:
                      if opt.model in  ['mobilenet', 'mobilenetv2', 'shufflenet', 'shufflenetv2']:
                          ...
                          if opt.model=='resnext' and opt.is_pretrained_model_has_dropout:
                              if opt.resnext_dropout:
                                model.module.fc[1]=nn.Linear(model.module.fc[1].in_features, opt.n_finetune_classes)
                              else:
                                model.module.fc=nn.Linear(model.module.fc[1].in_features, opt.n_finetune_classes)
                          else:
                              if opt.resnext_dropout and opt.model=='resnext':
                                model.module.fc=nn.Sequential(
                                                nn.Dropout(opt.resnext_dropout_prob),
                  ...
                  model=pretrain   #### because i saved whole model instead model state dict.   
        
        if isinstance(pretrain,dict) and not opt.no_train:
...
def modify_kernels(opt, model, modality):
    if modality == 'RGB' and opt.model not in ['c3d', 'squeezenet', 'mobilenet','shufflenet', 'mobilenetv2', 'shufflenetv2']:
       # print("[INFO]: RGB model is used for init model")
        if opt.model=='resnext' and not opt.with_egogesture:
